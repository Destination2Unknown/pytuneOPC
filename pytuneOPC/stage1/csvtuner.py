...
def main():
    ...
    def step2(filename):
        if bool(fname.get()) == True:
            ...
            t1,y1 = signal.step(sys1, N=int(df['PV'].count()))
            t1=t1[::]
            y1=y1[::]

            #Rescale
            plotpv=df['PV'].iloc[::].reset_index(drop=True)
            plotcv=df['CV'].iloc[::].reset_index(drop=True)

            #plot 
            plt.figure()
            plt.xlim(0,df['PV'].count()*1.1)
            plt.plot(plotpv, color="blue", linewidth=3, label='PV')
            plt.plot(t1+(i_start+(float(modeldt.get()))),((StartCV-InitCV)*y1)+round(float(ambient.get()),2),color="red",linewidth=3,label='Model')
            plt.plot(plotcv, color="green", linewidth=3, label='CV')
            plt.hlines(round(float(ambient.get()),2), 0, i_start+float(modeldt.get()),colors='red', linestyles='solid',linewidth=3,label='')
    ...
    def step3(filename):
        if bool(fname.get())==True:
            ...
            #Trim Timescale
            actualPV=df['PV'].iloc[max(0,indexofstart-1)::].reset_index(drop=True)
            actualCV=df['CV'].iloc[max(0,indexofstart-1)::].reset_index(drop=True)
    ...
    tk.Label(root, text="Bias:").grid(row=4,column=1,columnspan=1,padx=1,pady=1,sticky="W")
